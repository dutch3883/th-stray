...
export type UserRole = "admin" | "rescuer" | "user";
...
export const OPERATION_CONFIG: Record<string, OperationConfig> = {
  createReport: {
    allowedRoles: ["admin", "rescuer", "user"],
    requiresAuth: true,
  },
  listMyReports: {
    allowedRoles: ["admin", "rescuer", "user"],
  ...
  cancel: { allowedRoles: ["admin", "rescuer", "user"], requiresAuth: true },
...
export async function checkAuthorization(
  ...
  if (config.allowedRoles.length > 0) {
    ...
    if (!userRole || !config.allowedRoles.includes(userRole as UserRole)) {
